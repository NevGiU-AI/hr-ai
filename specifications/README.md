...
| Production security | Authentication and tenant foundation implemented | Email/password login, bcrypt hashes, server sessions, CSRF, route protection, four roles, tenant row isolation, admin-only built-in CV import, and tenant-scoped account lifecycle administration are implemented. Throttling, security audit logs, password management, shared sessions, malware scanning, and retention controls remain. |

...
| Production security | Authentication and tenant foundation implemented | Email/password login, bcrypt hashes, server sessions, CSRF, route protection, four roles, tenant row isolation, admin-only built-in CV import, and tenant-scoped account listing/creation are implemented. Role updates, account disabling, session revocation, throttling, security audit logs, malware scanning, and retention controls remain. |
